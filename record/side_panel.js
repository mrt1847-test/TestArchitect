...
const inspectedTabId = null; // 사이드 패널에서는 devtools API를 사용할 수 없음
...
function withActiveTab(callback) {
  ...
  // 사이드 패널에서는 모든 창에서 활성 탭을 찾되, 일반 브라우저 창의 탭만 선택
  // 사이드 패널 창이 아닌 실제 브라우저 창의 탭을 찾기 위해 windowType 확인
    ...
    const windowPromises = tabs.map(tab => {
      return new Promise((resolve) => {
        chrome.windows.get(tab.windowId, (win) => {
          ...
          // 'normal' 타입 창의 탭만 선택 (side_panel, devtools 등 제외)
...
function requestAiSelectorsForEvent(event, eventIndex) {
  ...
  const requestContext = {
    ...
    language: selectedLanguage,
    aiModel: aiSettings.model || '',
    tabId: null // 사이드 패널에서는 withActiveTab에서 처리
  };
...
// 사이드 패널이 로드될 때 현재 활성 탭의 URL에서 tcId와 projectId 자동 추출
(function initAutoFillFromUrl() {
  ...
  function checkAndAutoFillIds() {
    // 1순위: chrome.storage에서 저장된 파라미터 읽기 (Content Script 또는 Background가 저장한 값)
    chrome.storage.local.get(['testArchitectParams', 'recordingData'], (result) => {
      // testArchitectParams 또는 recordingData 중 하나라도 있으면 사용
      const params = result.testArchitectParams || result.recordingData;
      
      if (params) {
        console.log('[SidePanel] 저장된 파라미터 읽기:', params);
        ...
        if (params.tcId && tcIdInput && !tcIdInput.value) {
          ...
          if (!isNaN(tcIdNum) && tcIdNum > 0) {
            tcIdInput.value = tcIdNum;
            console.log('[SidePanel] TC ID 자동 설정:', tcIdNum);
          }
        }
        ...
        if (params.projectId && projectIdInput) {
          ...
          if (!isNaN(projectIdNum) && projectIdNum > 0) {
            ...
            if (!projectIdInput.value || projectIdInput.value === '1') {
              projectIdInput.value = projectIdNum;
              console.log('[SidePanel] Project ID 자동 설정:', projectIdNum);
            }
          }
        }
